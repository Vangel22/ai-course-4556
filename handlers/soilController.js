...
exports.chatAboutSoils = async (req, res) => {
  try {
    ...
    const context = soils.map(
      (s) =>
        `Name: ${s.name}, Type: ${s.type}, pH: ${s.ph}, Humus: ${s.humus}, Location: ${s.location}, Culture: ${s.culture}`
    );

    const systemMessage =
      "You are an expert about soils in Macedonia. Use the following information to answer for the questions:";
...
exports.addSampleSoils = async (req, res) => {
  try {
    ...
    const inserted = await Soils.insertMany(sampleSoils);
